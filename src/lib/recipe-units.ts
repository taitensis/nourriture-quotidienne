...
const U: Record<CanonUnit, UnitDef> = {
  ...
  tsp: { dim: "volume", toBase: n => n*5,    fromBase: n => n/5,    aliases: ["tsp","teaspoon","teaspoons","cac","càc","c.à.c","c a c","c-a-c","cuil. à café"], group:"spoons" },
  tbsp:{ dim: "volume", toBase: n => n*15,   fromBase: n => n/15,   aliases: ["tbsp","tablespoon","tablespoons","cas","càs","c.à.s","c a s","c-a-s","cuil. à soupe"], group:"spoons" },
};

// --- parsing helpers (supports unicode fractions, "1 1/2", "400–500") ---
const UNICODE_FRAC: Record<string, number> = { "¼":0.25,"½":0.5,"¾":0.75,"⅓":1/3,"⅔":2/3,"⅛":0.125,"⅜":0.375,"⅝":0.625,"⅞":0.875 };
...
function toNiceFraction(n: number): string {
  ...
  let best = 0, diff = 1;
  opts.forEach(v => { const d = Math.abs(frac - v); if (d < diff) { diff = d; best = v; }});
...
function ratioToString(v: number): string {
  const MAP: Record<number,string> = { 0.125:"1/8",0.1667:"1/6",0.2:"1/5",0.25:"1/4",0.3333:"1/3",0.375:"3/8",0.5:"1/2",0.625:"5/8",0.6667:"2/3",0.75:"3/4",0.8333:"5/6",0.875:"7/8" };
...
function chooseVolumeUnit(baseMl: number, origin?: CanonUnit): { unit: CanonUnit, qty: number } {
  ...
  if (baseMl >= 100 && baseMl < 1000) return { unit: "dl", qty: U.dl.fromBase(baseMl) };
  if (baseMl >= 10 && baseMl < 100)  return { unit: "cl", qty: U.cl.fromBase(baseMl) };
...
export function scaleAndNormalize(qty: number, unit: CanonUnit | null, factor: number) {
...
export function scaleIngredientString(str: string, factor: number): string {
  ...
  // format number: fractions for spoons & small volumes; decimals for others
